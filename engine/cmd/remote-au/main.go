...
func run(args []string, stdout, stderr io.Writer) error {
	opts := globalOptions{
		...
		logLevel:  "info",
		logFormat: "text",
		backend:   audio.DefaultBackendName(),
	}
	...
	switch remaining[0] {
	case "devices":
		return runDevices(remaining[1:], stdout, backend)
	case "selftest":
		return runSelftest(remaining[1:], stdout, backend, format, effectiveDebug, logger)
	case "recv":
...
func registerGlobalFlags(fs *flag.FlagSet, opts *globalOptions) {
	...
	fs.StringVar(&opts.backend, "backend", opts.backend, fmt.Sprintf("audio backend (%v)", audio.Backends()))
}
...
func runDevices(args []string, stdout io.Writer, backend audio.Backend) error {
	...
	lists, err := backend.EnumerateDevices()
	if err != nil {
		return err
	}
	if jsonOutput {
		return audio.EncodeDeviceListsJSON(stdout, lists)
	}
	return audio.PrintDevices(stdout, lists)
}

func runSelftest(args []string, stdout io.Writer, backend audio.Backend, format audio.Format, verbose bool, logger logging.Logger) error {
	if len(args) != 0 {
	...
	capture, err := backend.OpenCapture(audio.CaptureOptions{
		Format:  format,
	...
	playback, err := backend.OpenPlayback(audio.PlaybackOptions{
		Format: format,
		Pull:   func(out []byte, _ uint32) int { return capture.Read(out) },
	})
	if err != nil {
		return err
	}
...
func runRecv(args []string, stdout, stderr io.Writer, backend audio.Backend, format audio.Format, verbose bool, logger logging.Logger) error {
	...
	playbackOpts := audio.PlaybackOptions{
		Format: format,
		Pull: func(out []byte, frameCount uint32) int {
			// The mixer always fills the buffer (silence when starved).
			receiver.Pull(out, frameCount)
			return len(out)
		},
	}
	if deviceSelectorRequestsSelection(deviceSelector) {
		playbackOpts.DeviceSelector = deviceSelector
		fmt.Fprintf(stdout, "playback device: %s\n", deviceSelector)
	}
...
func runSend(args []string, stdout, stderr io.Writer, backend audio.Backend, format audio.Format, verbose bool, logger logging.Logger) error {
	...
	if deviceSelectorRequestsSelection(deviceSelector) {
		captureOpts.DeviceSelector = deviceSelector
	}

	capture, err := backend.OpenCapture(captureOpts)
	if err != nil {
		return err
	}
	defer func() {
...
func parseCaptureSource(name string) (audio.Source, error) {
	switch name {
	case "mic", "microphone":
