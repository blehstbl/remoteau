...
func TestDevicesJSONUsesMachineCleanStdout(t *testing.T) {
	backend := &fakeBackend{lists: audio.DeviceLists{
		Playback:     []audio.DeviceInfo{{Index: 0, Name: "Output", ID: "play-0"}},
		Capture:      []audio.DeviceInfo{{Index: 1, Name: "Mic", ID: "cap-1"}},
		LoopbackNote: "note",
	}}
	oldOpen := openBackend
	openBackend = func(string) (audio.Backend, error) { return backend, nil }
	t.Cleanup(func() {
		openBackend = oldOpen
	})

	var stdout, stderr bytes.Buffer
	if err := run([]string{"devices", "--json"}, &stdout, &stderr); err != nil {
	...
	if strings.Contains(stdout.String(), "Playback devices:") {
		t.Fatalf("stdout includes human text: %q", stdout.String())
	}
}

func TestDefaultBackendIsUsed(t *testing.T) {
	backend := &fakeBackend{lists: audio.DeviceLists{}}
	oldOpen := openBackend
	openBackend = func(name string) (audio.Backend, error) {
		if name != audio.DefaultBackendName() {
			t.Fatalf("openBackend(%q), want default %q", name, audio.DefaultBackendName())
		}
		return backend, nil
	}
	t.Cleanup(func() {
	...
	var stdout, stderr bytes.Buffer
	if err := run([]string{"devices"}, &stdout, &stderr); err != nil {
...
func (f *fakeBackend) OpenCapture(audio.CaptureOptions) (audio.Capture, error) {
	return nil, audio.ErrNoBackend
}
func (f *fakeBackend) OpenPlayback(audio.PlaybackOptions) (audio.Playback, error) {
	return nil, audio.ErrNoBackend
}
