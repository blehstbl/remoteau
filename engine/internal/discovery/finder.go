	...
	for {
		...
		if err != nil {
			if errors.Is(err, net.ErrClosed) {
				// The shutdown watcher closed the socket (caller deadline or
				// cancellation). Instances found so far are valid results:
				// return them unless we have nothing and the context is done.
				if peers := peerInstancesToPeers(instances); len(peers) > 0 {
					return peers, nil
				}
				if err := ctx.Err(); err != nil {
					return nil, err
